...
the License.
*/
import { html, SVGTemplateResult, TemplateResult, CSSResult, PropertyValueMap, css } from 'lit';
...
export default class AnypointChipInputElement extends AnypointInputElement {
  static get styles(): CSSResult[] {
    return [
      ...
      .highlight {
        background: var(--list-highlight-background, rgba(0, 0, 0, 0.12));
      }
      `,
    ];
  }
  ...
  _autocompleteTemplate(): TemplateResult | string {
    ...
    return html`<anypoint-autocomplete
      .target="${this.inputElement as HTMLInputElement | undefined}"
      .source="${this.source}"
