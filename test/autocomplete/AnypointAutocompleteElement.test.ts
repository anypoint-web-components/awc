...
describe('AnypointAutocompleteElement', () => {
  const suggestions: string[] = ['Apple', 'Apricot', 'Avocado', 'Banana'];
  const objectSuggestions: Suggestion[] = [
    {
      value: 'Apple',
      id: 1,
    },
  ...
  const htmlSuggestions: Suggestion[] = [
    {
      value: 'Apple',
  ...
  function notifyInput(target: EventTarget): void {
    target.dispatchEvent(new Event('input', { composed: true, bubbles: true }));
  }

  async function basicFixture(): Promise<AnypointAutocomplete> {
    return fixture(html`<anypoint-autocomplete></anypoint-autocomplete>`);
  }

  async function suggestionsFixture(): Promise<HTMLDivElement> {
    return fixture(html`
      <div>
        <input id="f1" />
  ...
  async function modelFixture(model: Suggestion[]): Promise<HTMLDivElement> {
    return fixture(html`
      <div>
        <input id="f1" />
  ...
  async function loaderFixture(): Promise<HTMLDivElement> {
    return fixture(html`
      <div>
        <input id="f2" />
  ...
  async function stringTargetFixture(): Promise<HTMLDivElement> {
    return fixture(html`<div><input id="f2">
    <anypoint-autocomplete target="f2"></anypoint-autocomplete></div>`);
  }

  async function anypointFixture(): Promise<HTMLDivElement> {
    return fixture(html`<div><input id="f2">
    <anypoint-autocomplete anypoint target="f2"></anypoint-autocomplete></div>`);
  }

  async function noTargetValueUpdateFixture(): Promise<HTMLDivElement> {
    return fixture(html`
      <div>
        <input id="f1" />
        <anypoint-autocomplete noAnimations target="f1" .source="${suggestions}"></anypoint-autocomplete>
  ...
  describe('Initialization', () => {
    ...
    it('has default loader', () => {
      assert.isUndefined(element.loader);
    });

    it('has default _loading', () => {
      assert.isUndefined(element._loading);
    });
    ...
    it('has default openOnFocus', () => {
      assert.isUndefined(element.openOnFocus);
    });
  ...
  describe('Suggestions with label', () => {
    ...
    it('renders HTML template label', async () => {
      ...
      notifyInput(input);
      await nextFrame();
      const label = element._listbox!.querySelector('anypoint-item b') as HTMLElement;
  ...
  describe('_targetChanged()', () => {
    ...
    it('removes listeners from old target', async () => {
      ...
      const element = region.querySelector('anypoint-autocomplete')!;
      const i1 = (region.querySelector('#i1') as HTMLInputElement);
      element.target = 'i2';
      const spy = sinon.spy();
      element.addEventListener('query', spy);
      i1.value = 'test';
    ...
    it('adds listeners to new target', async () => {
      ...
      const element = region.querySelector('anypoint-autocomplete')!;
      const i2 = (region.querySelector('#i2') as HTMLInputElement);
      element.target = 'i2';
      const spy = sinon.spy();
      element.addEventListener('query', spy);
      i2.value = 'test';
  ...
  describe('_filterSuggestions()', () => {
    ...
    it('returns all when no query', async () => {
      ...
      element._filterSuggestions();
      assert.typeOf(element.suggestions, 'array');
      const mapped = (element.source.map((item, index) => {
    ...
    it.skip('sorts the results #1', async () => {
      ...
      element._filterSuggestions();
      assert.equal((element.suggestions![0] as InternalSuggestion).value, 'ab');
    });
