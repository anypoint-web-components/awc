...
describe('<anypoint-button>', () => {
  ...
  async function roleFixture(): Promise<AnypointButtonElement> {
    return fixture(
      html`<anypoint-button role="link">Button</anypoint-button>`
    );
  }

  async function tabindexFixture(): Promise<AnypointButtonElement> {
    return fixture(html`<anypoint-button tabindex="-1">Button</anypoint-button>`);
  }

  async function togglesFixture(): Promise<AnypointButtonElement> {
    return fixture(html`<anypoint-button toggles>Button</anypoint-button>`);
  }

  async function noinkFixture(): Promise<AnypointButtonElement> {
    return fixture(html`<anypoint-button noink>Button</anypoint-button>`);
  }

  async function highEmphasisFixture(): Promise<AnypointButtonElement> {
  ...
  describe('High emphasis state', () => {
    let element: AnypointButtonElement;
    beforeEach(async () => {
      element = await highEmphasisFixture();
    });

    it('has the default elevation', () => {
  ...
  describe('Ripple effect', () => {
    ...
    it.skip('dispatched transitionend event on ripple end', async () => {
      element = await basicFixture();
      await keyDownUp(element, 'Space');
      const e = (await oneEvent(element, 'transitionend')) as TransitionEvent;
